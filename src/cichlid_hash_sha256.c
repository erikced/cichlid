...
static void            cichlid_hash_sha256_calc(CichlidHashSha256 *self, const char *buf, size_t bytes_read);
static gboolean        cichlid_hash_sha256_equals(uint32_t* a, uint32_t* b);
static void            cichlid_hash_sha256_final(CichlidHashSha256 *self);
static uint32_t       *cichlid_hash_sha256_get_hash(CichlidHash *object);
static char           *cichlid_hash_sha256_get_hash_string(CichlidHash *object);
static void            cichlid_hash_sha256_update(CichlidHash *object, const char *data, size_t data_size);
...
G_DEFINE_TYPE_WITH_CODE(CichlidHashSha256, cichlid_hash_sha256, G_TYPE_OBJECT,
...
cichlid_hash_interface_init(CichlidHashInterface *iface)
{
	...
	iface->get_hash = cichlid_hash_sha256_get_hash;
	iface->get_hash_string = cichlid_hash_sha256_get_hash_string;
	iface->update = cichlid_hash_sha256_update;
...
{
	CichlidHashSha256 *self = CICHLID_HASH_SHA256 (gobject);
 ...
 */
static void
cichlid_hash_sha256_calc(CichlidHashSha256 *self, const char *data, size_t bytes_read)
{
	...
	{
		CHANGE_ENDIANNESS(w, data+j*64, 16);
...
static void
cichlid_hash_sha256_final(CichlidHashSha256 *self)
{
	char     *buf;
	goffset   size_offset;
	uint32_t  tmp;
	...
	if (!self->hash_computed)
	{
		...
		cichlid_hash_sha256_calc(self, buf, size_offset + 8);
 ...
 */
static uint32_t *
cichlid_hash_sha256_get_hash(CichlidHash *object)
{
	CichlidHashSha256	*self;
	uint32_t		    *hash;

	self = CICHLID_HASH_SHA256(object);

	cichlid_hash_sha256_final(self);
	
	hash = g_malloc(SHA256_HASH_LENGTH/2);
	memcpy(hash, self->h, SHA256_HASH_LENGTH/2);
 ...
 */
static char *
cichlid_hash_sha256_get_hash_string(CichlidHash *object)
{
	char *hash_string;
	uint32_t *hash;

	hash = cichlid_hash_sha256_get_hash(object);
	if (hash == NULL)
		return NULL;

	hash_string = g_malloc(sizeof(char)*(SHA256_HASH_LENGTH + 1));
...
static void
cichlid_hash_sha256_update(CichlidHash *object, const char *data, size_t data_size)
{
	CichlidHashSha256 *self;
	char              *buf = NULL;
	...
	if (!self->data_left_size)
	{
		...
		cichlid_hash_sha256_calc(self, data, data_size - self->data_left_size);
	}
	...
	else
	{
		...
		cichlid_hash_sha256_calc(self, buf, data_size + self->data_left_size - new_data_left_size);
